...
function up_recipe_summary_render_cb($atts, $content, $block)
{
  ...
  foreach ($postTerms as $key => $term) {
    ...
    $cuisines .=
      "<a href='{$url}' target='_blank'>{$term->term}</a>{$comma}";
  }
  ...
  <div class="wp-block-udemy-plus-recipe-summary">
    ...
    <div class="recipe-columns-2-alt">
      ...
      <div class="recipe-metadata">
        ...
        <div class="recipe-data" id="recipe-rating" data-post-id="<?php echo $postID; ?>" data-avg-rating="<?php echo $rating; ?>" data-logged-in="<?php echo is_user_logged_in(); ?>" data-rating-count="<?php echo $ratingCount; ?>"></div>
